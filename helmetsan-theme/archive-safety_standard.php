<?php
/**
 * Safety standard archive template.
 *
...
<section class="hs-section">
    <header class="hs-section__head">
        ...
        <p>Global safety standards and certification references.</p>
    </header>
    ...
    <?php if (have_posts()) : ?>
        <div class="helmet-grid">
            <?php while (have_posts()) : the_post(); 
                $title = get_the_title();
                $desc = get_the_excerpt() ?: "Learn about the " . $title . " safety protocol.";
                $link = get_permalink();
                $img = get_the_post_thumbnail_url(get_the_ID(), 'large') ?: ($themeDir . '/assets/images/hubs/safety_standard/' . get_post_field('post_name') . '.png');
                
                // Fallback for image
                if (!file_exists(get_stylesheet_directory() . '/assets/images/hubs/safety_standard/' . get_post_field('post_name') . '.png') && !has_post_thumbnail()) {
                    $img = $hero_img;
                }
            ?>
                <a href="<?php echo esc_url($link); ?>" class="hs-discovery-card">
                    <article class="hs-discovery-card__inner">
                        ...
                        <div class="hs-discovery-card__content">
                            ...
                            <span class="hs-discovery-card__cta">View Protocol <svg viewBox="0 0 24 24"><path d="M5 12h14m-7-7 7 7-7 7"/></svg></span>
                        </div>
                    </article>
                </a>
            <?php endwhile; ?>

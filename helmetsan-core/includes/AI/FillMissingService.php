...
final class FillMissingService
{
     ...
     * Now supports meta_query for targeted brand/category reports.
     *
     * @return array{total_posts: int, fields: array<string, array{set: int, empty: int, pct: float}>, score: float}
     */
    public function getCoverageReport(string $postType, int $limit = 0, array $metaQuery = []): array
    {
        $fillable = FillableFieldsConfig::forPostType($postType);
        $taxConfig = FillableFieldsConfig::taxonomyFillableConfig()[$postType] ?? [];
        ...
        $totalSets = 0;
        $totalCheckpoints = $total * count($fields);
        ...
        foreach ($ids as $postId) {
            ...
            foreach (array_keys($taxConfig) as $t) {
                ...
                if (is_array($terms) && $terms !== []) {
                    $fields['taxonomy:' . $t]['set']++;
                    $totalSets++;
                } else {
                    $fields['taxonomy:' . $t]['empty']++;
                }
            }
        }
        ...
        $overallScore = $totalCheckpoints > 0 ? round(($totalSets / $totalCheckpoints) * 100, 1) : 0.0;
